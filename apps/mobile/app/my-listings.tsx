  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async (): Promise<MyListing[]> => {
      const { data, error } = await supabase
        .from('listings')
        .select('id, title, price, city, photos, is_active')
        .eq('user_id', user!.id)
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {isLoading ? (
        ...
        <View style={styles.centered}>
          <Text style={styles.errorText}>Could not load your listings.</Text>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.centered}>
              ...
              <Text style={styles.emptyBody}>Tap the + on Home to list your place.</Text>
            </View>
          }
          renderItem={({ item }) => (
