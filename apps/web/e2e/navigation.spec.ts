...
test.describe('Navigation', () => {
  ...
  test.describe('Top Navbar', () => {
    ...
    test('should open profile dropdown on click', async ({ page }) => {
      ...
      skipIfNotAuthenticated(page)
      const nav = page.locator('nav')
      // Click the last button in the navbar (avatar/profile button)
      const profileBtn = nav.getByRole('button').last()
      await profileBtn.click()
      await page.waitForTimeout(500)
      // Should show dropdown with Profile and Sign out options
      const hasProfile = await page.getByText(/profile/i).first().isVisible().catch(() => false)
      const hasSignOut = await page.getByText(/sign out|logout|log out/i).first().isVisible().catch(() => false)
      const hasSettings = await page.getByText(/settings/i).first().isVisible().catch(() => false)
      expect(hasProfile || hasSignOut || hasSettings).toBeTruthy()
    })
  })
})
