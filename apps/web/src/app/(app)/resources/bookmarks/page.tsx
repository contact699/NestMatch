      ...
      {bookmarks.length === 0 ? (
        <Card variant="bordered">
          <CardContent className="py-12 text-center">
            <Bookmark className="h-12 w-12 mx-auto text-outline mb-4" />
        ...
        <div className="space-y-4">
          {bookmarks.map((bookmark) => {
            ...
            return (
              <div
                key={bookmark.id}
                className="group flex items-start gap-4 bg-surface-container-lowest ghost-border rounded-xl p-4 hover:border-outline-variant/30 transition-colors"
              >
                <div
                  className={`
                    w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0
                    ${isResource ? 'bg-primary-fixed' : 'bg-secondary-container'}
                  `}
                >
                  {isResource ? (
                ...
                <div className="flex-1 min-w-0">
                  {isResource ? (
                    <Link
                      ...
                      className="block"
                    >
                      <h3 className="font-medium text-on-surface hover:text-primary transition-colors">
                        {resource.title}
                      </h3>
                      {resource.excerpt && (
                ...
                <button
                  ...
                  className="p-2 text-outline hover:text-error hover:bg-error-container rounded-lg transition-colors opacity-0 group-hover:opacity-100"
