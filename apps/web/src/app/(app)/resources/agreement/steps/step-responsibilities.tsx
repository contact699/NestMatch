...
export function StepResponsibilities({ register, watch, setValue, errors }: StepResponsibilitiesProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-display font-semibold text-on-surface mb-1">Responsibilities</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {CLEANING_SCHEDULES.map((schedule) => (
            <button
              ...
              `}
            >
              <p className="font-medium text-sm text-on-surface">{schedule.label}</p>
              <p className="text-xs text-on-surface-variant">{schedule.desc}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      {cleaningSchedule === 'assigned' && cleaningAreas.length > 0 && (
        <div className="p-4 bg-surface-container rounded-xl space-y-3">
          <p className="text-sm font-medium text-on-surface-variant">Assign areas to roommates:</p>
          {cleaningAreas.map((area, index) => (
            <div key={index} className="flex items-center gap-3">
              <span className="text-sm text-on-surface-variant flex-1">{area.area}</span>
              <select
                value={area.assignedTo || ''}
                onChange={(e) => updateAreaAssignment(index, e.target.value)}
                className="w-40 px-3 py-2 text-sm ghost-border rounded-lg bg-surface-container-lowest text-on-surface"
              >
        ...
        <div className="space-y-3">
          {SHARED_SUPPLIES.map((approach) => (
            <label
              key={approach.value}
              className={`
                flex items-start p-3 rounded-xl ghost-border cursor-pointer transition-colors
            ...
            >
              ...
              />
              <div>
                <p className="font-medium text-sm text-on-surface">{approach.label}</p>
                <p className="text-xs text-on-surface-variant">{approach.desc}</p>
              </div>
            </label>
          ))}
        </div>
